...
### TRY TO AUGMENT WITH THE NEW DATA FROM THE SPREADSHEET


new_data = pd.read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vSQ3suQTRlJwGSc3qhAthlLeyfw--UOkRLurBG1lO8Mh-ShOqJqBUVQ-sATPXkG1mIvMwaBfgJCvulo/pub?output=csv")
new_data['REPORT_DATE'] = pd.to_datetime(new_data['REPORT_DATE']).dt.strftime("%Y-%m-%d")
new_data = new_data[['REPORT_DATE', 'CODE', 'ACTIVE_CNT', 'CASE_CNT', 'DEATH_CNT', 'TEST_CNT', 
'MED_HOSP_CNT', 'MED_ICU_CNT', 'NAME', 'PREV_ACTIVE_CNT', 'PREV_CASE_CNT', 'PREV_DEATH_CNT', 
'PREV_TEST_CNT', 'PREV_MED_HOSP_CNT', 'PREV_MED_ICU_CNT', 'NEW_CASE_CNT', 'PREV_NEW_CASE_CNT', 
'LAST_UPDATED_DATE']]

# %%

cops = new_data.copy()
# new_data['REPORT_DATE'] = new_data['REPORT_DATE'].dt.strftime("%Y-%m-%d")

# Get a subset of the data for every date in the spreadsheet
oldies = tog.loc[tog['REPORT_DATE'].isin(cops['REPORT_DATE'].unique().tolist())]
oldies = oldies[['REPORT_DATE', 'CODE', 'ACTIVE_CNT', 'CASE_CNT', 'DEATH_CNT', 'TEST_CNT',
 'MED_HOSP_CNT', 'MED_ICU_CNT', 'NAME', 'PREV_ACTIVE_CNT', 'PREV_CASE_CNT', 'PREV_DEATH_CNT',
  'PREV_TEST_CNT', 'PREV_MED_HOSP_CNT', 'PREV_MED_ICU_CNT', 'NEW_CASE_CNT', 'PREV_NEW_CASE_CNT',
   'LAST_UPDATED_DATE']]
...
for state in cops['CODE'].unique().tolist():
    for datto in cops['REPORT_DATE'].unique().tolist():
        oldies_inter = oldies.loc[(oldies['CODE'] == state) & (oldies['REPORT_DATE'] == datto)].copy()
        # print(oldies_inter.columns.tolist())
        # print(oldies_inter)
        cops_inter = cops.loc[(cops['CODE'] == state) & (cops['REPORT_DATE'] == datto)].copy()
        # print(cops_inter.columns.tolist())
        # print(cops_inter)
        tog_inter = oldies_inter.append(cops_inter)
        tog_inter.ffill(inplace=True)

        out.append(tog_inter)


fin_fix = pd.concat(out)


tog = tog.append(fin_fix)
tog = tog.drop_duplicates(subset=['REPORT_DATE', "CODE"], keep='last')
tog = tog.sort_values(by=["REPORT_DATE"], ascending=True)

### Keep copy

with open('archive/cases_feed_archive_AUGMENTED.csv', 'w') as f:
...
syncData(tog.to_dict(orient='records'),f'2022/01/oz-covid-health-data{testo}', f"cases")
# %%
